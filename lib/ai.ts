...
3. 循序渐进:每章 4-7 个步骤,先 lesson 讲概念(结合项目讲),再 challenge 动手练,quiz 放在每章末尾
4. 课程要成体系:每章 4-7 步,总步骤数 25-45 个;lesson 约占一半(内容要讲透,见 CHAPTER_TASK 的教学法),challenge 4-8 个,quiz 每章 1 个
...
8. 章节数遵循「最小可用版本」预算制,由学习目标与主题复杂度共同决定(6-14 章),课程要长而扎实:
   - 先划出完成目标「不可再减」的核心知识块序列,每个知识块 = 1 章;环境搭建、基础概念等铺垫内容合并进第 1 章,不单独成章;第 1 章结束时项目必须可运行、可见成果(最小可用版本)
   - 把目标拆细:每个核心知识块内部再按「做出来 → 做完善 → 做通用」拆出 1-3 个子阶段,每个子阶段 = 1 章,让课程像连载剧一样层层推进、步步有成果
   - 章节必须全部是构建阶段(像连续剧),不允许任何「注水」章;宁可每章做深,不要为了凑章节硬切碎内容
   - 目标宏大、技能面广(如「全栈开发」「爬虫+数据可视化」)→ 10-14 章;目标明确、入门小目标(如「给网页加一个按钮动效」)→ 6-8 章;中间情况 8-10 章
...
11. estimatedMinutes 与章节数严格对齐:每章学习时长 10-20 分钟,总时长 = 各章时长之和(不要脱离章节数凭空写总时长);课程体量越大单章分钟数取上限,越小取下限
12. 难度螺旋上升:章节顺序就是学习路径,每章只引入 1-2 个新概念,且只用前面章节已讲过的内容(不依赖未讲解的知识);最后 1-2 章必须是综合运用章,把前面所有知识点组合起来做一个完整功能,杜绝「学了不会用」
13. 代码示例规则:大纲步骤的 brief 若提到代码,必须指向项目中真实存在、可运行的写法,不得是伪代码或省略号
...
- 同一题内选项必须互不相同;不同题目之间的选项文字也不得完全重复;不得使用「以上都对/以上都不对」式技巧性选项
- correctIndex 必须在 options 下标范围内,正确答案唯一
- explanation 要说明正确选项为什么对,以及常见错误选项错在哪
- 交错回忆:quiz 中必须包含 1-2 道「回顾题」——考察本章之前章节教过的核心概念(不是本章内容),用新场景提问,让学习者区分新旧问题;回顾题放在 quiz 中间位置,不要全部堆在开头或结尾
...
2. bodyMarkdown 用简体中文,讲解要具体、有示例,循序渐进;lesson 必须 300 字以上(见教学法第 5 条结构),术语第一次出现时给出简短解释
...
教学法与课程生成完全一致:课程是真实项目,挑战是渐进小步,用种子代码(seedBefore/starterCode/seedAfter)保护结构;描述用「过渡句 + 概念 + 精确指令」三步式;测试用 freeCodeCamp hint 风格(每个测试只查一件事,失败信息写人话+细节提示,含防破坏断言,断言由宽到严)。lesson 必须讲透(300 字以上):过渡句 + 概念讲解(直觉/类比引出再给定义,术语加粗)+ 完整可运行的示例代码并解释关键行 + 常见坑 + 小结,内容充实不空洞。
