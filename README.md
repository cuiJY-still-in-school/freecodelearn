...
- **大纲微调**:生成前可编辑章节标题/目标描述、上移下移、删除章节,课程结构完全由你掌控
- **主题把关**:无关主题(如烹饪)会先被拦截提示,可选择「仍然生成」或「换个主题」
- **freeCodeCamp 式学习体验**:
  - 左侧大纲(章节可折叠、当前步骤自动跟随、挑战难度点一目了然)+ 右侧内容
  - 图文步骤式章节(markdown 渲染,代码块语言徽标 + 一键复制)
  ...
  - **3 级提示梯**:多次尝试失败后逐级解锁(思路方向 → 方法与语法骨架 → 接近实现),保护「生产性挣扎」
  - **章节测验 + 错题重做**:选择题含解析,答错只重做错题,全对才通过;数字键 1-9 选答案、Enter 提交
  - **间隔复习(Leitner)**:每道题进入 6h/1天/3天/7天/14天 复习箱,顶部「待复习」入口混合测验巩固记忆
  - 学习激励:通过横幅、章节完成转场、课程完成庆祝
  - 进度跟踪(localStorage,自动续学上次未完成步骤;首页卡片显示进度与「继续学习」)
  - 首页学习统计:今日完成步数 / 连续学习天数 / 累计步数
  - 追加章节:课程学习页面可直接让 AI 追加新章节
  - 刷新/返回自动恢复上次浏览位置;挑战编辑器内容持久化(切走/刷新不丢已写代码)
- **课程导入导出**:导出 .fcl 文件(含课程与学习进度);一键导出 Markdown 讲义(含代码/提示/答案)
- **深色模式**:设置页三态切换(跟随系统/浅色/深色),首帧无闪烁
- **键盘快捷键**:挑战页 Cmd/Ctrl+Enter 运行判题;课程页 ←/→ 切换步骤;quiz 数字键答题
...
| 平台 | 文件 | 安装方法 |
| --- | --- | --- |
| Linux | `FreeCodeLearn-1.3.1.AppImage` | 见下方 A 或 B |
| Linux | `freecodelearn_1.3.1_amd64.deb` | `sudo apt install ./freecodelearn_1.3.1_amd64.deb` |
| Windows | `FreeCodeLearn Setup 1.3.1.exe` | 双击运行,按向导安装 |
...
- A. 直接运行(需要 FUSE 支持):
  ...
  ```bash
  chmod +x FreeCodeLearn-1.3.1.AppImage
  ./FreeCodeLearn-1.3.1.AppImage
  ```

- B. 系统没有 FUSE 时,用解包模式运行:

  ```bash
  ./FreeCodeLearn-1.3.1.AppImage --appimage-extract-and-run
  ```
...
- **3 级提示梯 hints**:方向 → 方法/语法骨架 → 接近实现,尝试失败 ≥2 次才解锁
- **难度与概念元数据**:每挑战输出 difficulty(1-5,相邻步骤差 ≤1,随章节螺旋上升)与 concepts 标签,驱动侧边栏难度点与复习调度
- **主动回忆**:lesson 末尾「✍️ 自测一下」预测题,答案折叠在 details 块中
- **交错练习与脚手架渐隐**:章节内混入前序概念回顾问;同一概念连续 3 个挑战时种子代码完整度 80%→40%→仅签名
- **判题仿 hint 风格**:每个测试只查一件事;失败信息写人话 + 细节提示(拼写/大小写/空格);
  断言由宽到严(存在性 → 结构性 → 值宽松 → 值严格 → 规范性);含防破坏断言
- **测验考理解不考记忆**:用新例子检验概念;每章 quiz 混入 1-2 道前序章节回顾题
