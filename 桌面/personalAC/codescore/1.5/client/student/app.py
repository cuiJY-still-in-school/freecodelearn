...
class StudentApp:
    def __init__(self):
        ...
        self._last_srs_notify: float = 0.0    # timestamp
        self._last_todo_notify: float = 0.0   # timestamp
        self._notify_tick: int = 0            # counts 1-min ticks
        self._drive_sync: DriveSync | None = None
        self._focus_mode_active: bool = False
        self._study_seconds: int = 0     # 今日学习时长（学习类 category 累计）
        self._must_do_done: int = 0
        self._must_do_total: int = 0
        self._srs_count: int = 0
    ...
    def _pull_server_config(self):
        ...
        if cfg:
            ...
            self._focus_mode_active = bool(cfg.get('focus_mode', False))
            self._config.save()
    ...
    def _sync_activity(self):
        ...
        # 累计学习类时长
        from common.app_category import STUDY_CATEGORIES
        for r in records:
            if r.get('category') in STUDY_CATEGORIES:
                self._study_seconds += r.get('duration_seconds', 0)
    ...
    def _poll_config(self):
        ...
        self._must_do_done  = done
        self._must_do_total = total
        if self._pet_window:
        ...
        # 特别辅导模式变更
        focus_active = bool(cfg.get('focus_mode', False))
        if focus_active != self._focus_mode_active:
            self._focus_mode_active = focus_active
            if self._main_window:
                self._main_window.set_focus_mode(focus_active)
            if self._tray:
                msg = '特别辅导模式已开启，加油！' if focus_active else '特别辅导模式已结束'
                self._tray.notify('学习辅导', msg, QSystemTrayIcon.MessageIcon.Information)
    ...
    def _check_update(self):
        if not self._api:
            return
        from common.updater import check_update_async
        check_update_async(self._api, self._on_update_available)

    def _on_update_available(self, version: str, url: str):
        if self._tray:
            self._tray.notify(
                f'PersonalAC {version} 可更新',
                '有新版本可用，请向老师或家长申请更新',
    ...
    def _refresh_pet_status(self):
        if not self._pet_window:
            return
        h = self._study_seconds // 3600
        m = (self._study_seconds % 3600) // 60
        study_str = f'{h}小时{m}分' if h else (f'{m}分钟' if m else '刚开始')
        current_app = ''
        if self._monitor and self._monitor._current_session:
            current_app = self._monitor._current_session.app_name
        self._pet_window.update_status(
            study_time=study_str,
            task_done=self._must_do_done,
            task_total=self._must_do_total,
            srs_count=self._srs_count,
    ...
    def _check_notifications(self):
        ...
        try:
            ...
            for item in items:
                ...
                self._tray.notify(f"来自{who}的消息", caption, duration_ms=8000)
        except Exception as e:
            logger.debug("relay poll error: %s", e)

        # ── 2. SRS 复习提醒（每 30 分钟，间隔至少 2 小时）──────────────────
        if self._notify_tick % 30 == 0:
            try:
                count = self._api.get_srs_count()
                self._srs_count = count
                self._refresh_pet_status()
                now = time.time()
                if count > 0 and now - self._last_srs_notify >= 2 * 3600:
                    self._tray.notify(
                        '复习提醒',
                        f'有 {count} 个知识点待复习，趁热打铁效果更好！',
                        duration_ms=8000,
                    )
                    self._last_srs_notify = now
            except Exception as e:
                logger.debug('srs notify error: %s', e)

        # ── 3. 逾期待办提醒（每 60 分钟，间隔至少 4 小时）──────────────────
        if self._notify_tick % 60 == 0:
            try:
                now = time.time()
                if now - self._last_todo_notify >= 4 * 3600:
                    overdue = self._api.get_overdue_todos()
                    if overdue:
                        titles = "、".join(t.get("title", "待办") for t in overdue[:3])
                        suffix = f" 等 {len(overdue)} 项" if len(overdue) > 3 else ""
                        self._tray.notify(
                            "待办逾期",
                            f"{titles}{suffix}已超时，记得处理！",
                            duration_ms=10000,
                        )
                        self._last_todo_notify = now
            except Exception as e:
                logger.debug("todo notify error: %s", e)
    ...
    def _show_locked(self):
        ...
        self._main_window.set_focus_mode(self._focus_mode_active)
    ...
    def _on_blocked_app(self, process_name: str, window_title: str):
        logger.info('Blocked app detected: %s (%s)', process_name, window_title)
        if self._tray:
            self._tray.notify('检测到违规应用', f'已阻止：{process_name}')
        if sys.platform == 'win32':
            try:
                import psutil
                low = process_name.lower().replace('.exe', '')
