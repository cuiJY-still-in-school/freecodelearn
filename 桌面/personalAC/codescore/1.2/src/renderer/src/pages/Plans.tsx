...
function Plans(): React.ReactElement {
  ...
  const loadPlans = async (): Promise<void> => {
    setLoading(true)
    try {
      const activeResult = await planApi.getActive(STUDENT_ID)
      if (activeResult.success && activeResult.data) {
        setActivePlan(activeResult.data as Plan)
      }

      const listResult = await planApi.list(STUDENT_ID)
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    setSaving(true)
    try {
      const result = await planApi.create(STUDENT_ID, title.trim(), description.trim(), subjects)
      if (result.success) {
            ...
            {allPlans
              ...
              .map((p) => (
                <div
                  key={p.id}
                  className="card"
                  style={{ opacity: 0.7, borderLeft: '4px solid #e5e7eb' }}
                >
                  <div
                    style={{
                      display: 'flex',
