...
function Dashboard(): React.ReactElement {
  ...
  const [udpStatus, setUdpStatus] = useState<UdpBotStatus | null>(null)
  ...
  const loadDashboard = async (): Promise<void> => {
    ...
    try {
      // Active plan
      const planResult = await planApi.getActive(STUDENT_ID)
      ...
      const summaryResult = await dataApi.getSummary(STUDENT_ID, thirtyDaysAgo, now)
      if (summaryResult.success && summaryResult.data) {
        setSummary(summaryResult.data as SummaryData)
      }

      // Agent logs
      const logsResult = await agentApi.getLogs(STUDENT_ID, 5)
      if (logsResult.success && logsResult.data) {
        setLogs(logsResult.data as AgentLog[])
      }

      // Bot count
      const botsResult = await botApi.listBots()
      ...
      // UDP Bot status
      const udpResult = await udpApi.getStatus()
      if (udpResult.success && udpResult.data) {
        setUdpStatus(udpResult.data as UdpBotStatus)
      }
    } finally {
      setLoading(false)
    }
  ...
  const handleRunCycle = async (): Promise<void> => {
    ...
    try {
      await agentApi.runCycle(STUDENT_ID)
      setTimeout(() => {
        agentApi.getLogs(STUDENT_ID, 5).then((r) => {
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="grid-3" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
          ...
          {/* Workspace Usage */}
          <div>
            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>磁盘剩余</div>
            {workspaceStats ? (
              <>
                <div style={{ fontWeight: 600, fontSize: 15 }}>
                ...
                <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>
                  已用 {formatBytes(workspaceStats.usage)}
                  {workspaceStats.warning && (
                    <span style={{ color: '#f59e0b', marginLeft: 6 }}>⚠ 磁盘空间不足</span>
                  )}
                </div>
              </>
            ) : (
          ...
          {/* PAC UDP Bot Status */}
          <div>
            <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>PAC UDP Bot</div>
            {udpStatus ? (
              <>
                <div style={{ fontWeight: 600, fontSize: 15 }}>
                  {udpStatus.running ? (
                    <span style={{ color: '#10b981' }}>
                      运行中（端口 {udpStatus.port}）
                    </span>
                  ) : (
                ...
                {udpStatus.running && (
                  <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>
                    {udpStatus.sessions} 个连接
                    {udpStatus.uptime !== undefined && (
                      <span style={{ marginLeft: 6 }}>
                        · 已运行 {Math.round(udpStatus.uptime / 1000)} 秒
                      </span>
                    )}
                  </div>
                )}
