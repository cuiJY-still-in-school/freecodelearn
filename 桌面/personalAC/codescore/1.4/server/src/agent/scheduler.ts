...
export class AgentScheduler {
  ...
  private registerCronJob(
    ...
    const task = cron.schedule(cronExpression, () => {
      console.log(`Cron triggered: scheduleId=${scheduleId}, student=${studentId}`)
  ...
  createSchedule(
    ...
    try {
      ...
      console.log(`Schedule created: ${scheduleId} for student ${studentId}`)
      return {
        success: true,
  ...
  removeSchedule(scheduleId: string): SchedulerResult {
    try {
      ...
      const error = err instanceof Error ? err.message : String(err)
      console.error('removeSchedule error:', error)
      return { success: false, error }
    }
  }
  ...
  compensateMissedTasks(): void {
    try {
      ...
      console.log(`Compensating ${missedSchedules.length} potentially missed tasks`)

      for (const schedule of missedSchedules) {
        const lastRun = schedule.last_run || now - 24 * 60 * 60 * 1000

        if (this.shouldHaveFired(schedule.cron_expression, lastRun, now)) {
          console.log(`Compensating missed task: ${schedule.id} for student ${schedule.student_id}`)
          this.onScheduleTrigger(schedule.id, schedule.student_id, schedule.description + ' (补偿)')
        }
      }
    } catch (err) {
      console.error('compensateMissedTasks error:', err)
    }
  }
  ...
  destroy(): void {
    ...
    console.log('AgentScheduler destroyed')
  }
}
