...
export interface AgentContextData {
  ...
  sobriety: {
    ...
    persistentBlocks: Array<{ topic: string; subject: string }>
...
export class AgentContext {
  buildContext(studentId: string): AgentContextData {
    ...
    const studentName = userRow?.display_name ?? (studentId === 'superadmin' ? 'SuperAdmin' : studentId)
    ...
    const overdueTodos = db
      .prepare(
        `SELECT id, title, due_date, priority
         FROM Todo
         WHERE student_id = ? AND status = 'pending' AND due_date < ? AND delete_flag = 0
         ORDER BY due_date ASC
         LIMIT 5`
      )
      .all(studentId, now) as AgentContextData['overdueTodos']

    const upcomingTodos = db
      .prepare(
        `SELECT id, title, due_date, priority
         FROM Todo
         WHERE student_id = ? AND status = 'pending' AND due_date >= ? AND due_date <= ? AND delete_flag = 0
         ORDER BY due_date ASC
         LIMIT 5`
      )
      .all(studentId, now, now + 72 * 60 * 60 * 1000) as AgentContextData['upcomingTodos']

    const recentMistakes = db
      .prepare(
        `SELECT id, subject, topic, error_type, root_cause, reviewed, create_time
         FROM MistakeBook
         WHERE student_id = ? AND delete_flag = 0
         ORDER BY reviewed ASC, create_time DESC
         LIMIT 10`
      )
      .all(studentId) as AgentContextData['recentMistakes']

    // 学习连续天数（streak）— 从今天往前连续有记录的天数
    ...
    // 最近聊天话题（学生端最近几条用户消息，供 planner 避免重复推送）
    let recentChatTopics: string[] = []
    try {
      const rows = db.prepare(`
        SELECT content FROM ChatHistory
        WHERE user_id = ? AND role = 'user' AND delete_flag = 0
        ORDER BY create_time DESC LIMIT 5
      `).all(studentId) as Array<{ content: string }>
      recentChatTopics = rows
        .map(r => (typeof r.content === 'string' ? r.content : '').replace(/\s+/g, ' ').trim().slice(0, 80))
        .filter(t => t.length > 5)
    } catch { /* non-fatal */ }

    let sobriety: AgentContextData['sobriety'] = null
    try {
      const { getOrRefreshSnapshot } = require('../services/sobriety.service') as typeof import('../services/sobriety.service')
      const snap = getOrRefreshSnapshot(studentId, 30 * 60 * 1000)
      if (snap) {
        sobriety = {
          urgencyLevel: snap.urgency.level,
          todayPriority: snap.today_priority,
          examDaysLeft: snap.exam?.days_left ?? null,
          examType: snap.exam?.type ?? null,
          dueReviewsCount: snap.due_reviews.length,
          persistentBlocks: snap.persistent_blocks.map(b => ({ topic: b.topic, subject: b.subject })),
          daysSinceActive: snap.days_since_active,
          driftWarning: snap.subject_drift.drift_warning,
        }
      }
    } catch { /* snapshot unavailable is non-fatal */ }
