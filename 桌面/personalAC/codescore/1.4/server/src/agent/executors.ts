...
export const weaknessQuizExecutor: Executor = async (task, context, aiConfig, callAI) => {
  ...
  const targetTopics = new Set(targets.map(p => p.topic))
  const relevantMistakes = (context.recentMistakes ?? []).filter(m => m.reviewed === 0 && targetTopics.has(m.topic))
  const mistakeHint = relevantMistakes.length > 0
    ? `\n\n## 历史错题（优先针对这些失误点出题）\n${relevantMistakes.slice(0, 5).map(m =>
        `- [${m.subject}] ${m.topic}${m.error_type ? `（${m.error_type}）` : ''}${m.root_cause ? `：${m.root_cause}` : ''}`
      ).join('\n')}`
    : ''
...
export const scheduleExecutor: Executor = async (task, context, _aiConfig, _callAI) => {
  ...
  console.log(`Schedule executor: creating schedule "${description}" (${cron}) for student ${context.studentId}`)
...
// ─────────────────────────────────────────────
// Overdue Todo Analysis Executor
// ─────────────────────────────────────────────

export const overdueTodoExecutor: Executor = async (task, context, aiConfig, callAI) => {
  if (context.overdueTodos.length === 0) {
    return { success: true, taskType: task.type, output: { type: 'text', content: '当前无逾期待办。' } }
  }

  const todoList = context.overdueTodos
    .map((t) => {
      const days = Math.floor((Date.now() - t.due_date) / 86400000)
      return `- ${t.title}（逾期 ${days} 天，优先级：${t.priority}）`
    })
    .join('\n')

  const planInfo = context.activePlan
    ? `学习方向：${context.activePlan.title}，科目：${context.activePlan.subjects.join('、')}`
    : '暂无设定学习方向'

  const prompt = `你是一个学习辅助 AI，正在帮助一位学生分析逾期的待办事项。

学生信息：
${planInfo}

逾期待办（共 ${context.overdueTodos.length} 项）：
${todoList}

请完成以下两件事：
1. **逾期原因分析**：推测每项逾期的可能原因（学习任务过重？时间安排不合理？任务太模糊难以行动？）
2. **调整计划**：给出具体可行的调整建议（例如：拆解任务、重新排期、删除低价值项目）

要求：语气友好鼓励，不批评，300 字以内，用 Markdown 格式（标题 + 列表）输出。`
...
// ─────────────────────────────────────────────
// Proactive Encouragement Executor
// ─────────────────────────────────────────────

export const proactiveEncouragementExecutor: Executor = async (task, context, aiConfig, callAI) => {
  const daysSince = context.sobriety?.daysSinceActive ?? 3

  const prompt = `你是学习辅助 AI PersonalAC。学生 ${context.studentName} 已经 ${daysSince} 天没有学习记录了。
请生成一条友好、温暖、不说教的激励消息，唤起他们的学习热情。
...
- 150 字以内`
  ...
  return {
    ...
    taskType: task.type,
    output,
    sideEffects: [{ type: 'bot_message', userId: context.studentId, content: `👋 ${output.content}` }]
  }
}
...
export const examAlertExecutor: Executor = async (task, context, aiConfig, callAI) => {
  const examDaysLeft = context.sobriety?.examDaysLeft
  const examType = context.sobriety?.examType
  if (examDaysLeft === null || examDaysLeft === undefined || !examType) {
    return { success: false, taskType: task.type, error: '未找到考试信息' }
  }

  const prompt = `你是学习辅助 AI PersonalAC。${context.studentName} 距离「${examType}」还有 ${examDaysLeft} 天。
...
const registry: Partial<Record<string, Executor>> = {
  ...
  overdue_analysis: overdueTodoExecutor,
  proactive_encouragement: proactiveEncouragementExecutor,
  exam_alert: examAlertExecutor,
}
