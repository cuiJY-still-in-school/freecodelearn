export interface ToolContext {
  ...
  userRole: string   // 'guardian' | 'student'
  rawMessages?: Array<{ role: string; content: unknown }>  // 前端原始消息（含图片 base64）
  _callCounts?: Record<string, number>  // 每次请求的工具调用次数计数（防滥用）
}
...
let _openAICache: unknown[] | null = null
let _anthropicCache: unknown[] | null = null
...
export function toOpenAITools(): unknown[] {
  return (_openAICache ??= registry.map(t => ({
    type: 'function',
...
export function toAnthropicTools(): unknown[] {
  return (_anthropicCache ??= registry.map(t => ({
    name: t.name,
