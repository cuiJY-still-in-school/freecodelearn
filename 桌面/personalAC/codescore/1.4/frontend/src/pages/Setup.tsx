...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 480, background: 'var(--bg-card)', border: '1px solid var(--border)', borderTop: '3px solid var(--primary)', borderRadius: 12, padding: '40px 40px 36px' }}>
        {children}
      </div>
    </div>
  )
...
export default function Setup({ onDone }: Props): React.ReactElement {
  ...
  if (step === 'student') return (
    <Card>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 6 }}>
          {GRADES.map(g => (
            <button key={g} onClick={() => setGrade(grade === g ? '' : g)} style={{
              ...
              background: grade === g ? 'var(--primary-light)' : 'transparent',
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, margin: '6px 0 10px' }}>
          {suggestedSubjects.map(s => (
            <button key={s} onClick={() => toggleSubject(s)} style={{
              ...
              background: subjects.includes(s) ? 'var(--primary-light)' : 'transparent',
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button className="btn btn-secondary" onClick={() => setStep('guardian')} style={{ flex: 1, justifyContent: 'center' }}>返回</button>
  ...
  return (
    <Card>
      ...
      <button className="btn btn-secondary" onClick={copyToken} style={{ width: '100%', justifyContent: 'center', marginBottom: 12 }}>
        {copied ? '已复制' : '复制访问码'}
      </button>
