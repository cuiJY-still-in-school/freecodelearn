...
export interface StudentRecord {
  id: string; name: string; grade: string | null; token: string
}

export const authApi = {
  login: (syncToken: string) =>
    post<UserInfo>('/auth/login', { syncToken }),
...
export const settingsApi = {
  ...
  saveEmailConfig: (email: string, authCode: string, imapHost: string, imapPort: number) =>
    post('/settings/email', { email, authCode, imapHost, imapPort }),
  getEmailConfig: () => get('/settings/email'),
  testEmailConnection: () => post('/settings/email/test')
}
...
export const emailApi = {
  getStatus: () => get('/email/status'),
  startPolling: () => post('/email/start'),
  stopPolling: () => post('/email/stop')
}
...
export interface EmailConfig {
  email: string; imapHost: string; imapPort: number; configured: boolean
}
export interface EmailStatus {
  polling: boolean; lastCheck?: number; error?: string
}
