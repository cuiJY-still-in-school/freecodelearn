...
class EmailService {
  ...
  async init(): Promise<void> {
    ...
    if (!config) {
      console.log('EmailService: no config found, skipping init')
      return
    }
    console.log(`EmailService initialized for ${config.email}`)
  }
  ...
  startPolling(): void {
    if (this.polling) {
      console.log('EmailService: already polling')
      return
    }
    this.polling = true
    console.log('EmailService: starting polling (every 5 minutes)')

    // Run immediately
    this.checkNewEmails().catch((err) => {
      console.error('EmailService: initial check failed:', err)
    })

    this.intervalId = setInterval(() => {
      this.checkNewEmails().catch((err) => {
        console.error('EmailService: scheduled check failed:', err)
      })
    }, 5 * 60 * 1000)
  }
  ...
  stopPolling(): void {
    ...
    this.polling = false
    console.log('EmailService: polling stopped')
  }
  ...
  async checkNewEmails(): Promise<void> {
    ...
    try {
      ...
      console.log('EmailService: connected to IMAP')
      ...
      try {
        ...
        for await (const message of client.fetch('1:*', {
          ...
          console.log(`EmailService: processing message uid=${uid} from=${senderEmail} subject=${subject}`)
        ...
        if (seenUids.length > 0) {
          ...
          console.log(`EmailService: marked ${seenUids.length} emails as read`)
        }
      } finally {
        lock.release()
      }
      ...
      console.error('EmailService: checkNewEmails error:', error)
  ...
  private async processEmailSource(
    ...
    try {
      ...
      try {
        ...
        plainText = parsed.text || (typeof parsed.html === 'string' ? parsed.html.replace(/<[^>]+>/g, ' ') : '') || ''
      ...
      const workspaceRoot = path.join(process.env.DATA_DIR || path.join(process.cwd(), 'data'), 'workspace')
      ...
      for (const att of attachments) {
        ...
        console.log(`EmailService: saved attachment ${savedFilename}`)
      ...
      if (attachments.length === 0 && plainText.trim().length > 0) {
        console.log(`EmailService: email from ${senderEmail} has no attachments, text only`)
      }
    } catch (err) {
      console.error('EmailService: processEmailSource error:', err)
    }
  }
  ...
  private registerFileIndex(params: {
    ...
    try {
      ...
      console.error('EmailService: registerFileIndex error:', err)
    }
  }
  ...
  private registerResource(params: {
    ...
    try {
      ...
      if (!admin) {
        console.warn('EmailService: no admin user found, cannot register resource')
        return
      }
      ...
      console.error('EmailService: registerResource error:', err)
    }
  }
  ...
  private notifyAgent(senderEmail: string, fileName: string, relativePath: string): void {
    try {
      ...
      console.warn('EmailService: failed to notify agent:', err)
    }
  }
