...
class WorkspaceService {
  ...
  constructor() {
    this.workspaceRoot = path.join(process.env.DATA_DIR || path.join(process.cwd(), 'data'), 'workspace')
  }
  ...
  async getDiskInfo(): Promise<{ diskFree: number; diskTotal: number }> {
    try {
      ...
      console.warn('getDiskInfo: statfs failed:', err)
  ...
  async write(
    ...
    try {
      ...
      console.log(`Workspace write: ${absolutePath}`)
  ...
  async delete(
    ...
    try {
      ...
      console.log(`Workspace delete: ${absolutePath}`)
