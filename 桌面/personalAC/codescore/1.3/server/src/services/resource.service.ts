...
export async function upload(
  ...
  try {
    ...
    console.log(`Resource uploaded: ${fileName} by superadmin`)
...
export function deleteResource(resourceId: string): ResourceResult {
  try {
    const db = getDB()
    const now = Date.now()
    ...
    ).run(SUPERADMIN_ID, now, resourceId)
