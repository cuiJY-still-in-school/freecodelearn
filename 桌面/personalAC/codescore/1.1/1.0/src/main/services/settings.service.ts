...
export async function getModels(): Promise<SettingsResult> {
  try {
    ...
    const allModels: ModelsDevEntry[] = response.data

    // Filter models that support multimodal (vision/image input)
    const multimodalModels: AIModel[] = []

    for (const model of allModels) {
      const inputCaps: string[] = model.capabilities?.input || []
      const supportsVision =
        inputCaps.includes('image') ||
        inputCaps.includes('vision') ||
        inputCaps.includes('multimodal')
      ...
      const aiModel: AIModel = {
        ...
        pricing: model.pricing?.prompt !== undefined
          ? { prompt: model.pricing.prompt, completion: model.pricing.completion || 0 }
          : undefined
      }

      multimodalModels.push(aiModel)
    }
