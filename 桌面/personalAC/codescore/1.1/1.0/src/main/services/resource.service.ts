...
export async function upload(
  ...
  try {
    ...
    if (user.role !== 'teacher' && user.role !== 'admin') {
      return { success: false, error: '只有教师或管理员可以上传资源' }
    }
    ...
    db.prepare(`
      INSERT INTO Resource (id, uploader_id, file_name, file_path, file_type, file_size, subject, parsed_text, create_user, update_user, create_time, update_time, delete_flag)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
    `).run(
      resourceId,
      user.id,
    ...
    try {
      ...
      if (engine) {
        engine.handleEvent({ type: 'new_resource', resourceId, subject, uploaderId: user.id })
      }
    } catch {
      // Agent not ready
    }
