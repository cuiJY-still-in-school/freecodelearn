...
async function initEmailService(): Promise<void> {
  try {
    ...
    // If email is already configured, start polling automatically
    const emailConfig = getRawEmailConfig()
    if (emailConfig) {
      log.info('Email config found, starting polling automatically')
      emailService.startPolling()
    } else {
      log.info('No email config found, skipping email polling')
    }
  } catch (err) {
    log.error('Failed to initialize email service:', err)
  }
}
